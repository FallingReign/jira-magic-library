...
describe('actionable failure messages', () => {
  ...
  it('preserves a string upload failure in the attachment error', async () => {
    const client = { postMultipart: jest.fn().mockRejectedValue('Upload unavailable') };
    await expect(new AttachmentUploader(client as any).uploadForIssue('/attachments', [], 'TEST-1'))
      .rejects.toMatchObject({ name: 'AttachmentUploadError', message: 'Issue TEST-1 attachment upload failed: Upload unavailable' });
  });
